...
public class SqlGeneratorTestMine {
    ...
    @Test
    public void testGenerateFromInsertEnum() throws Exception {
        final Map<String, Object> context = new HashMap<>();
        final WmAdTableInsert[] tables = WmAdTableInsert.values();
        final List<Table> lt = new ArrayList<>();
        for (final WmAdTableInsert table : tables) {
            lt.add(table.get());
        }
        context.put("tables", lt);
    ...
    private void prepareinsert(final Table table) {
        table.add(new Column("col1"));
        table.add(new Column("col2"));
    }
